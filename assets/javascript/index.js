...
const carouselFunctionality = function(dataSelector) {
  const selectorStringRoot = '.carousel[data-carousel="' + dataSelector + '"]';
  const carousel = document.querySelector(selectorStringRoot);
  const carouselClose = document.querySelector(selectorStringRoot + ' .carousel__close');
  const carouselLeft = document.querySelector(selectorStringRoot + ' .carousel__left');
  const carouselRight = document.querySelector(selectorStringRoot + ' .carousel__right');
  const carouselImages = document.querySelectorAll(selectorStringRoot + ' .carousel__image');
  const carouselDots = document.querySelectorAll(selectorStringRoot + ' .carousel__dot');
  const hammertime = new Hammer(carousel);
  const removeActives = function() {
    for (let j = 0; j < carouselImages.length; j++) {
      carouselImages[j].classList.remove('carousel__image--active');
    }
    for (let j = 0; j < carouselDots.length; j++) {
      carouselDots[j].classList.remove('carousel__dot--active');
    }
  }
  const addActive = function() {
    carouselImages[activeIndex].classList.add('carousel__image--active');
    carouselDots[activeIndex].classList.add('carousel__dot--active');
  }
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  const carouselOpeners = document.querySelectorAll('.carousel__image--opener');
  let navTimer;
  ...
  if (portfolioButton && portfolioDropdown) {
    body.addEventListener('touchend', function(event) {
      if (event.target && (event.target.id == 'portfolio-button' || event.target.classList.contains('nav__dropdown-link'))) {
        return;
      }
      portfolioDropdown.classList.remove('nav__dropdown--active');
    });
    portfolioButton.addEventListener('touchstart', function() {
